...
#define CUSPARSE_CHECK(statement)                                                   \
    do                                                                              \
    {                                                                               \
        cusparseStatus_t result = (statement);                                      \
        if (CUSPARSE_STATUS_SUCCESS != result)                                      \
        {                                                                           \
            fprintf(stderr, "[%s:%d] CUSPARSE failed\n", __FILE__, __LINE__);       \
            exit(-1);                                                               \
        }                                                                           \
        assert(CUSPARSE_STATUS_SUCCESS == result);                                  \
    } while (0)
